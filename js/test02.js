...
function selectCountry(index) {
    ...
    currenIndex = provice[current2.prov].city[current2.city].districtAndCounty.length;
      for(var i=0;i<currenIndex;i++){
        addrWrap.getElementsByTagName('li')[i].style.backgroundColor = '';
      }
      addrWrap.getElementsByTagName('li')[index].style.backgroundColor = '#FFC300';
...
document.getElementById('title-wrap').onclick = function (e) {
    var e = e || window.event;
    //                  谷歌支持       IE支持    这里是是不是冒泡 this冒泡  这里不冒泡
    var target = e.target || e.srcElement;
    if (target && target.nodeName == 'LI') {
        for (var z = 0; z < 3; z++) {
